...
def get_schedule(num, rad=4):
  """ draw points from [0, 1] but dense in both ends """
  if num == 1:
...
class Model(object):
  ...
  def ais(self, x, schedule):
    """
    :param x: sample points drawn from target distribution
    """
    w = 0.0
    ...
    for t0, t1 in zip(schedule[:-1], schedule[1:]):
      ...
      #                                        tf.reshape(self.generator(self.z), [self.num_samples, self.batch_size, self.generator.output_dim]),
      #                                        self.sigma),
