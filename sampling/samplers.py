...
def run_ais(model, obs, num_samples, num_steps, sigma, hdim, L, epsilon, data, prior, schedule=None, model_name='vae10'):
    if schedule is None:
    ...
    if prior[:5] == 'recog':
        ...
        if prior[5] == '1':
            rec_net = utils.load_encoder(model_name, 'c4000', eval_np=True)
        elif prior[5] == '2':
